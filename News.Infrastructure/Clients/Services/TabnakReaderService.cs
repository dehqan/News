...
{
    public class TabnakReaderService : IReaderService
    {
        private readonly IApiService _apiService;
        private readonly ILogger<TabnakReaderService> _logger;

        public TabnakReaderService(IApiService apiService, ILogger<TabnakReaderService> logger)
        {
        ...
        {
            var data = await _apiService.SendRequest<TabnakRssResult>(ApiMethodEnum.GET, ApiSerializerEnum.XML, url);
        ...
        public async Task<List<ReaderResult>> GetRssDetails(List<RssResult> dataList)
        {
            ...
            foreach (var rssResult in dataList)
            {
                ...
                {
                    _logger.LogWarning(e, $"Client: Isna, Link: {rssResult.Link}");
                    continue;
                }
            }
