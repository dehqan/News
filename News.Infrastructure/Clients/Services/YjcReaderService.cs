...
using News.Infrastructure.Clients.Models.Isna;
using News.Infrastructure.Clients.Models.Mehrnews;
using News.Infrastructure.Clients.Models.Tabnak;
using News.Infrastructure.Clients.Models.Tasnimnews;
using News.Infrastructure.Clients.Models.Yjc;

namespace News.Infrastructure.Clients.Services
{
    public class YjcReaderService : IReaderService
    {
        private readonly IApiService _apiService;
        private readonly ILogger<YjcReaderService> _logger;

        public YjcReaderService(IApiService apiService, ILogger<YjcReaderService> logger)
        {
        ...
        {
            var data = await _apiService.SendRequest<YjcRssResult>(ApiMethodEnum.GET, ApiSerializerEnum.XML, url);
        ...
        public async Task<List<ReaderResult>> GetRssDetails(List<RssResult> dataList)
        {
            ...
            foreach (var rssResult in dataList)
            {
                try
                {
                    ...
                    var body = doc.DocumentNode.SelectSingleNode("//div[contains(@class, 'body')]");
                ...
                {
                    _logger.LogWarning(e, $"Client: Yjc, Link: {rssResult.Link}");
                    continue;
                }
            }
