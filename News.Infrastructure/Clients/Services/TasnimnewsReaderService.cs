...
namespace News.Infrastructure.Clients.Services
{
    public class TasnimnewsReaderService : IReaderService
    {
        ...
        private readonly ILogger<TasnimnewsReaderService> _logger;

        public TasnimnewsReaderService(IApiService apiService, ILogger<TasnimnewsReaderService> logger)
        {
        ...
        public async Task<List<RssResult>> GetRssData(string url)
        {
            ...
            return data.Channel.Item.Select(x => new RssResult
            {
                ...
                Image = x.Content.Url.Trim(),
                Thumbnail = x.Thumbnail.Url.Trim()
            }).ToList();
        }
        ...
        public async Task<List<ReaderResult>> GetRssDetails(List<RssResult> dataList)
        {
            ...
            foreach (var rssResult in dataList)
            {
                ...
                {
                    _logger.LogWarning(e, $"Client: Tasnimnews, Link: {rssResult.Link}");
                    continue;
                }
            }
